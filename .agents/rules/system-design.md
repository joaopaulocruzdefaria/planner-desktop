...
## 2. Typography Rules
Readability is the absolute priority for a text editor app.
* **Font Family:** Use system sans-serif fonts by default to feel native on Windows/Mac, or fallback to `Inter` (if explicitly installed). Do NOT import heavy Google Fonts.
* **Scale:** Use strict Tailwind utility classes:
  * Regular Body: `text-base leading-relaxed` (16px)
  * Small UI text (Sidebar, metadata): `text-sm leading-tight` (14px)
  * Heading 1 (Page Title): `text-4xl font-bold tracking-tight`
...
* **Buttons & Inputs:** Keep them subtle. Prefer `ghost` styles (no background until hovered) over solid backgrounds to maintain the minimalist vibe. Standard padding for UI elements: `px-3 py-1.5` or `px-4 py-2`.
